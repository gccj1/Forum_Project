...
public class AccountServiceImpl extends ServiceImpl<AccountMapper, Account> implements AccountService {
     ...
     */
    public String emailVerifyCode(String type, String email, String address){
     ...
     */
    @Override
    public String resetEmailAccountPassword(passResetVO info) {
    ...
    public String resetConfirm(ConfirmResetVO info) {
        String email = info.getEmail();
        String code = this.getEmailVerifyCode(email);
        if(!this.existsAccountByEmail(email)) return "该邮件地址未被注册";
        if(code == null) return "请先获取验证码";
        if(!code.equals(info.getCode())) return "验证码错误，请重新输入";
        return null;
    }
    ...
    @Override
    public String emailModify(int id, ConfirmResetVO info) {
        String email = info.getEmail();
        String code = this.getEmailVerifyCode(email);
        if(code == null) return "请先获取验证码";
        if(!code.equals(info.getCode())) return "验证码错误，请重新输入";
    ...
    public Account findAccountById(int id) {
        return this.query().eq("uid", id).one();
    }
